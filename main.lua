...
-- ====================================================================
-- 🔐 GITHUB VERİ BAĞLANTISI (KENDİ BİLGİLERİNİ GİR)
-- ====================================================================
local githubKullaniciAdi = "BURAYA_GITHUB_KULLANICI_ADIN"
local repoAdi = "BURAYA_REPO_ADIN"
-- ====================================================================

-- GitHub'dan anlık şifre listesini çekme motoru
...
if not success or type(keyList) ~= "table" then
    LP:Kick("SW HUB: Şifre veri tabanına bağlanılamadı! GitHub linklerini kontrol edin.")
    return
end

local loginSuccess = false
local loginWindowActive = true
local isAdmin = false -- Giriş yapan kişinin şifre üretme yetkisi var mı?

-- 🛑 ŞİFRE PANELİ (Parantez içi ve ipuçları tamamen silindi)
...
SubmitBtn.MouseButton1Click:Connect(function()
    ...
    if keyList[girilenSifre] ~= nil then
        loginSuccess = true
        isAdmin = keyList[girilenSifre] -- Şifrenin yanındaki true/false yetkisini alırsın
...
-- 🔑 SADECE YETKİSİ OLANLAR İÇİN KEY GENERATOR SEKMESİ
if isAdmin then
    ...
    GenTab:CreateButton({
        ...
        Callback = function()
            ...
            Rayfield:Notify({Title = "Şifre Üretildi", Content = "Yeni Şifre: " .. sonuc .. " (Panoya Kopyalandı)", Duration = 5})
            setclipboard(sonuc) -- Otomatik telefona kopyalar
        end,
    })
    
    GenTab:CreateToggle({
        Name = "Bu Şifreyi Alan Yeni Şifre Üretebilsin mi? (Admin)",
        CurrentValue = false,
        Flag = "AccessToggle",
        Callback = function(Value)
            GiveGeneratorAccess = Value
        end,
    })
    
    GenTab:CreateButton({
        Name = "ŞİFREYİ GITHUB DEPOSUNA EKLE & AKTİF ET",
        Callback = function()
            ...
            -- Bilgilendirme Metni
            print("Yeni Şifre Formadı: ['" .. GeneratedKeyStr .. "'] = " .. tostring(GiveGeneratorAccess))
            Rayfield:Notify({
                Title = "Kopyalandı!",
                Content = "GitHub'daki 'key_data.lua' dosyasına eklemeniz için satır panoya kopyalandı.",
                Duration = 6
            })
...
-- ANA MOTOR KODLARI (Aynen Korundu)
